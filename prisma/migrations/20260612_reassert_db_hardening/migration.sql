-- Re-assert 20260227_db_hardening.
--
-- That migration is recorded as applied in _prisma_migrations on prod, but
-- none of its objects exist there (verified 2026-06-12: no
-- users_email_active_unique, no exercises/plans partial unique indexes, no
-- workout-completion trigger, no orderIndex check). It was most likely
-- baselined or run against a different database. The statements are
-- idempotent, so re-running them everywhere is safe.
--
-- Prerequisite: no active duplicate rows may exist for these indexes.
-- Prod was cleaned via prisma/cleanup_duplicate_users.ts before this
